...
const AdminStudentAccounts = () => {
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            try {
                await axios.delete(`http://localhost:8080/Requests/student/${selectedStudentNumber}/all`, {
                    headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` },
                });
            } catch (error) {
                if (error.response?.status === 404) {
                } else {
                    console.warn("Error deleting clearance requests:", error.message);
                }
            }
    
            try {
                await axios.delete(`http://localhost:8080/Student/student/${selectedStudentNumber}`, {
                    headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` },
                });
            } catch (error) {
                console.error("Error deleting student record:", error.message);
                throw new Error("Failed to delete student record.");
            }
    
            try {
                await axios.delete(`http://localhost:8080/user/delete/${selectedStudentNumber}`, {
                    headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` },
                });
            } catch (error) {
                if (error.response?.status === 404) {
                } else {
                    console.warn("Error deleting user account:", error.message);
                }
            }
            ...
            console.error("Deletion process encountered an error:", error.message);
            showAlert("An error occurred while deleting the student. Please try again.", "error");
        } finally {
